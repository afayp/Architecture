package com.example.mvpdagger.model;
...
/**
 * model层，实现序列化接口Serializable或者Parcelable。
 * 并不会直接把该javabean对象set到xml中与view绑定，进行展示。
 * 通常是在viewmodel层中通过网络或者存储得到javabean，
 * 然后将Javabean中的字段与viewmodel中的属性进行一定的逻辑绑定，
 * 最后在xml中与view绑定的是viewmodel层对象。
 */
